...
| **Scheduled dispatcher API** | `api/send-scheduled-notifications.ts` — on-demand serverless route that queries all active subscriptions (`user_push_subscriptions` where `enabled = true`), then fetches due, unsent items from the `events`, `goals`, and `tasks` tables (`alert_timestamp`/`alertTimestamp <= NOW()` AND `alert_sent` is not `true`) and dispatches **context-aware** Web Push payloads via `web-push`: `Event: {title}` / `Starts at {time}`, `Goal Reminder: {title}` / `Time to check in on your goal!`, and `Task Due: {title}` / `Due at {time}`. Each item is marked `alert_sent = true` after a successful send so duplicate cron runs never resend the same notification. Expired endpoints (HTTP 404/410) are automatically deleted. Returns `{ success, notificationsSent, timestamp }` |
...
- **Context-Aware Notification Dispatch & Duplicate Prevention** — reworked `api/send-scheduled-notifications.ts` to query due, unsent items directly from the `events`, `goals`, and `tasks` tables (tolerating both `alert_timestamp`/`alertTimestamp` and `alert_sent` naming) instead of the generic `alert_notifications` queue. Payloads are now type-aware: `Event: {title}` / `Starts at {time}`, `Goal Reminder: {title}` / `Time to check in on your goal!`, and `Task Due: {title}` / `Due at {time}`. After a successful Web Push send, the item is marked `alert_sent = true` so overlapping cron runs never dispatch the same notification twice.
