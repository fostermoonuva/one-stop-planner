...
| **Budget transactions** | Category, amount, description, date, type (expense/income/transfer), account, payment method (debit/credit/cash), credit card paid status, flow type (spending/saving/investing/income), from/to account IDs for transfers |
...
- **Inter-Account Transfers & Savings Account Type** — added a `Transfer` transaction type alongside `Expense` and `Income`. When Transfer is selected, the form shows dual **From Account** and **To Account** selectors with validation preventing same-account transfers. Executing a transfer dynamically decreases the source account balance and increases the destination account balance in real-time, while being excluded from monthly income/expense totals. Transfers render with a teal arrow indicator (e.g., `Checking ➔ HYSA Savings`) and support optional category tagging for tracking intent. Added `Savings` as an officially recognized account type with a teal/emerald accent badge, included in liquid net worth calculations. Database schema updated with `from_account_id`/`to_account_id` columns, a `transfer` type check constraint, and a `chk_transfer_accounts_differ` constraint ensuring source ≠ destination.
